...
class NOAAETLManager:
    ...
    def process_request(self):
        ...
        if self.args['Call_Direct_Download'] == 'CSV' || self.args['Call_Direct_Download'] == 'JSON':
    ...
    ##Below are data checking calls. They all default to True in a typical user request.
        ...
        #to-do. Check if parameters are valid and get station list based on bounding box
        if self.args['Call_Parameter_Check']:
            pass
        ...
        if self.args['Call_Fill_Incomplete']:
            ...
            if complete == False:
                ...
                thread = threading.Thread(target=self.fill_incomplete, args=(arg_trans, self.args['API_Arguments'], self.args['NOAA_API_KEY'], conn))
    ...
    def aggregate_data(self, df, translation, additional_arguments):
        ...
        for datatype in df['datatype'].unique():
            ...
            aggregated = grouped.agg({'value': agg_method})
        ...
        return_columns = additional_arguments.get('return_columns', result_df.columns)
    ...
    def check_completeness(self, db_vals, api_vals):
        ...
        ##report and return True
            ...
            self.response_codes['check_completeness'] = True
            return True
        ...
        ##report and return False
            ...
            self.response_codes['check_completeness'] = False
            return False
    ...
    def fill_incomplete(self, translation, api_parameters, noaa_api_key, conn):
        ...
        #attempt to connect to database and push data
        try:
            #start cursor with database connection
            cur = conn.cursor()
            #add data to database row by row
            for row in api_vals:
                #generate a unique ID (uid) for each row, which is a combination of date, station, and datatype
                uid = str(row['date']) + '_' + str(row['station']) + '_' + str(row['datatype'])
                #add the row into the database using the below sql query.
                ##SKIPS VALUES IF THEY ARE ALREADY IN THE DATABASE
                cur.execute("""
                    INSERT INTO noaa_api(date, datatype, station, attributes, value, uid)
                    VALUES (%s, %s, %s, %s, %s, %s)
                    ON CONFLICT (uid) DO NOTHING;
                """, (row['date'], row['datatype'], row['station'], row['attributes'], row['value'], uid))
            #report to console that all rows added
            print('rows added, committing now')
            #finish pushing data
            conn.commit()
        #if there is an error, report it to console
        except psycopg2.Error as e:
                print(f"Error executing SQL: {e}")
        #close cursor and database connection, ending the thread.
        finally:
            cur.close()
            conn.close()